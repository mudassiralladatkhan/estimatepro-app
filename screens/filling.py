...
class SlopeFillingScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.length_input = styled_input('Embankment Length (m)')
        self.width_input = styled_input('Embankment Width (m)')
        self.height_input = styled_input('Embankment Height (m)')
        self.trip_volume_input = styled_input('Volume per Trip (m³)')
    ...
    def calculate(self, instance):
        try:
            ...
            self.result_label.text = f"Volume: {volume:.2f} m³ | Trips: {trips} | Estimated Cost: ₹{cost:,.2f}"
        except ValueError:
