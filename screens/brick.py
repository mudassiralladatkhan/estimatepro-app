...
class BrickScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.brick_length = styled_input('Brick Length (cm)')
        self.brick_height = styled_input('Brick Height (cm)')
        self.cost_per_brick = styled_input('Cost per Brick (₹)')

        content.add_widget(self.wall_length)
        content.add_widget(self.wall_height)
        content.add_widget(self.brick_length)
        content.add_widget(self.brick_height)
        content.add_widget(self.cost_per_brick)
    ...
    def calculate(self, instance):
        try:
            ...
            brick_length = float(self.brick_length.text)
            brick_height = float(self.brick_height.text)
            cost_per_brick = float(self.cost_per_brick.text)

            wall_area_cm2 = wall_length * wall_height * 10000
            brick_area_cm2 = brick_length * brick_height
            bricks_needed = (wall_area_cm2 / brick_area_cm2) * 1.05
            total_cost = bricks_needed * cost_per_brick

            self.result_label.text = f"Bricks Needed: {int(bricks_needed)} | Estimated Cost: ₹{total_cost:,.2f}"
        except ValueError:
    ...
    def clear(self, instance):
        ...
        self.brick_length.text = ''
        self.brick_height.text = ''
        self.cost_per_brick.text = ''
