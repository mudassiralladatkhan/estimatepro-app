...
class PlasterScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.mortar_ratio = styled_input('Mortar Ratio (e.g. 4 for 1:4)')
        self.bag_price = styled_input('Cement Bag Price (₹)')

        content.add_widget(self.wall_length)
        content.add_widget(self.wall_height)
        content.add_widget(self.mortar_ratio)
        content.add_widget(self.bag_price)
    ...
    def calculate(self, instance):
        try:
            ...
            ratio = float(self.mortar_ratio.text)
            price = float(self.bag_price.text)

            area = length * height
            cement_bags = area * (0.18 / ratio)
            cost = cement_bags * price

            self.result_label.text = f"Cement Bags: {cement_bags:.2f} | Estimated Cost: ₹{cost:,.2f}"
        except ValueError:
